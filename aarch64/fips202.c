...
#include <assert.h>
#include "fips202.h"
#include "asm/macro/KeccakP-1600-SnP.h"
...
static inline void keccak_absorb_asm(uint64_t *s,
                          unsigned int r,
                          const unsigned char *m, unsigned long long int mlen,
                          unsigned char p)
{
...
static inline void keccak_squeezeblocks_asm(unsigned char *h, unsigned long long int nblocks,
                                 uint64_t *s, unsigned int r)
{
  while(nblocks > 0) 
  {
...
{
  keccak_squeezeblocks_asm(output, nblocks, s, SHAKE128_RATE);
}
              ...
              const unsigned char *input,  unsigned long long inlen)
{
  ...
  /* Absorb input */
  keccak_absorb_asm(s, SHAKE256_RATE, input, inlen, 0x1F);

  /* Squeeze output */
  keccak_squeezeblocks_asm(output, nblocks, s, SHAKE256_RATE);
  ...
  if(outlen) 
  {
    keccak_squeezeblocks_asm(output, 1, s, MIN(outlen, SHAKE256_RATE));
  }
}
