...
int crypto_kem_enc_asm(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{
  ...
  shake256_asm(buf,2*NEWHOPE_SYMBYTES,buf,NEWHOPE_SYMBYTES + 1);                     /* Don't release system RNG output */

  cpapke_enc_asm(ct, buf, pk, buf+NEWHOPE_SYMBYTES);                                 /* coins are in buf+NEWHOPE_SYMBYTES */

  shake256_asm(ss, NEWHOPE_SYMBYTES, buf, NEWHOPE_SYMBYTES);                         /* hash pre-k to ss */
  return 0;
}
...
{
  cpapke_dec_asm(ss, ct, sk);

  shake256_asm(ss, NEWHOPE_SYMBYTES, ss, NEWHOPE_SYMBYTES);                          /* hash pre-k to ss */
