...
* Returns 0 (success)
**************************************************/
int crypto_kem_keypair_asm(unsigned char *pk, unsigned char *sk)
{
  size_t i;

  cpapke_keypair_asm(pk, sk);                                                   /* First put the actual secret key into sk */
  sk += NEWHOPE_CPAPKE_SECRETKEYBYTES;
...
* Returns 0 (success)
**************************************************/
int crypto_kem_enc_asm(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{
  unsigned char k_coins_d[3*NEWHOPE_SYMBYTES];                                                /* Will contain key, coins, qrom-hash */
  unsigned char buf[2*NEWHOPE_SYMBYTES+1];
  int i;
  ...
  cpapke_enc_asm(ct, buf+1, pk, k_coins_d+NEWHOPE_SYMBYTES);                                      /* coins are in k_coins_d+NEWHOPE_SYMBYTES */
...
* On failure, ss will contain a randomized value.
**************************************************/
int crypto_kem_dec_asm(unsigned char *ss, const unsigned char *ct, const unsigned char *sk)
{
  int i, fail;
  unsigned char ct_cmp[NEWHOPE_CCAKEM_CIPHERTEXTBYTES];
  unsigned char buf[2*NEWHOPE_SYMBYTES+1];
  ...
  buf[0] = 0x08;
  cpapke_dec_asm(buf+1, ct, sk);

  for(i=0;i<NEWHOPE_SYMBYTES;i++)                                                             /* Use hash of pk stored in sk */
    buf[1+NEWHOPE_SYMBYTES+i] = sk[NEWHOPE_CCAKEM_SECRETKEYBYTES-2*NEWHOPE_SYMBYTES+i];
  shake256(k_coins_d, 3*NEWHOPE_SYMBYTES, buf, 2*NEWHOPE_SYMBYTES+1);

  cpapke_enc_asm(ct_cmp, buf+1, pk, k_coins_d+NEWHOPE_SYMBYTES);                                  /* coins are in k_coins_d+NEWHOPE_SYMBYTES */
