...
void poly_uniform_asm(poly *a, const unsigned char *seed)
{
  unsigned int ctr=0;
  uint16_t val;
  uint64_t state[25];
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* generate a in blocks of 64 coefficients */
  {
    ctr = 0;
    extseed[NEWHOPE_SYMBYTES] = i; /* domain-separate the 16 independent calls */
    shake128_absorb_asm(state, extseed, NEWHOPE_SYMBYTES+1);
    while(ctr < 64) /* Very unlikely to run more than once */
    {
      shake128_squeezeblocks_asm(buf,1,state);
      for(j=0;j<SHAKE128_RATE && ctr < 64;j+=2)
      {
        val = (buf[j] | ((uint16_t) buf[j+1] << 8));
        if(val < 5*NEWHOPE_Q)
...
void poly_ntt_asm(poly *r)
{
  mul_coefficients_asm(r->coeffs, gammas_bitrev_montgomery);
  ntt_asm((uint16_t *)r->coeffs, gammas_bitrev_montgomery);
}
...
void poly_invntt_asm(poly *r)
{
  bitrev_vector_asm(r->coeffs);
  ntt_asm((uint16_t *)r->coeffs, omegas_inv_bitrev_montgomery);
  mul_coefficients_asm(r->coeffs, gammas_inv_montgomery);
}
