...
static inline void gen_a_asm(poly *a, const unsigned char *seed)
{
  poly_uniform_asm(a,seed);
}
...
void cpapke_keypair_asm(unsigned char *pk,
                    unsigned char *sk)
{
  poly ahat, ehat, ahat_shat, bhat, shat;
  unsigned char z[2*NEWHOPE_SYMBYTES];
  ...
  z[0] = 0x01;
  randombytes(z+1, NEWHOPE_SYMBYTES);
  shake256_asm(z, 2*NEWHOPE_SYMBYTES, z, NEWHOPE_SYMBYTES + 1);
  ...
  poly_mul_pointwise_asm(&ahat_shat, &shat, &ahat);
  poly_add_asm(&bhat, &ehat, &ahat_shat);
...
void cpapke_enc_asm(unsigned char *c,
                const unsigned char *m,
                const unsigned char *pk,
                const unsigned char *coin)
{
  ...
  poly_mul_pointwise_asm(&uhat, &ahat, &sprime);
  poly_add_asm(&uhat, &uhat, &eprime);

  poly_mul_pointwise_asm(&vprime, &bhat, &sprime);
  poly_invntt_asm(&vprime);

  poly_add_asm(&vprime, &vprime, &eprimeprime);
  poly_add_asm(&vprime, &vprime, &v); // add message
...
void cpapke_dec_asm(unsigned char *m,
                const unsigned char *c,
                const unsigned char *sk)
{
  poly vprime, uhat, tmp, shat;
  ...
  decode_c(&uhat, &vprime, c);
  poly_mul_pointwise_asm(&tmp, &shat, &uhat);
  poly_invntt_asm(&tmp);

  poly_sub_asm(&tmp, &tmp, &vprime);
